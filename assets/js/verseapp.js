...
var json, //Eliminates redundant server calls by globally caching the data
	verseNumber, //used to easily pass "id" numbers for functions
	debug = false, //turn on debugging

	//configuration
	stockPack = "assets/SummerPack.json"; //url to the stock pack that comes with the app
...
function fixHeight(adjustment) {
	//subtract the header and nav size 
	var newHeight = window.innerHeight - 52 - $("#knownButton").outerHeight() - adjustment + "px";
	$(".versecontainer").css("height", newHeight);
}
...
function populate(storageName, packSectionName) {
	if (localStorage[storageName]) { //Check that the data loaded properly
		var jsonObject = JSON.parse(localStorage[storageName]),
			up = JSON.parse(localStorage.userProfile),

			//bug fix for issue with height when there are no verses in the queue
			counter = 0,
			theme, icon;

		if (localStorage.currentTheme == "a" || localStorage.currentTheme == "c" || localStorage.currentTheme == "d" || localStorage.currentTheme == "e" || localStorage.currentTheme == "g") {
			theme = "assets/img/dark/";

		} else if (localStorage.currentTheme == "b" || localStorage.currentTheme == "f") {
			theme = "assets/img/light/";
		}
		//generate the verses for this pack
		for (var key in jsonObject["data"]) {
			var ident = jsonObject["data"][key].id;
			for (var num in up["userProfile"][storageName]) {
				if (up["userProfile"][storageName][num].id == ident) {
					...
					if (packSectionName == "all") {
						if (queue == "A") {
							$("#versecontainer").append('<div class="verse" data-role="collapsible" data-collapsed-icon="" data-expanded-icon="" data-inset="false" style="margin: 0px;"><h4><center>' + jsonObject["data"][key].reference + ' (' + jsonObject["data"][key].version + ')</center><a onclick="toggleVerseMenu(' + ident + '); "><img src="' + theme + 'menu.png" class="menu" /></a></h4><p>' + jsonObject["data"][key].text + '</p></div>').trigger('create');
							counter++;
						} else if (queue == "W") {
							$("#versecontainer").append('<div class="verse" data-role="collapsible" data-collapsed-icon="" data-expanded-icon="" data-inset="false" style="margin: 0px;"><h4><center><img src="' + theme + 'working_icon.png" class="icons" />' + jsonObject["data"][key].reference + ' (' + jsonObject["data"][key].version + ')</center><a onclick="toggleVerseMenu(' + ident + '); "><img src="' + theme + 'menu.png" class="menu" /></a></h4><p>' + jsonObject["data"][key].text + '</p></div>').trigger('create');
							counter++;
						} else if (queue == "K") {
							$("#versecontainer").append('<div class="verse" data-role="collapsible" data-collapsed-icon="" data-expanded-icon="" data-inset="false" style="margin: 0px;"><h4><center><img src="' + theme + 'known_icon.png" class="icons" />' + jsonObject["data"][key].reference + ' (' + jsonObject["data"][key].version + ')</center><a onclick="toggleVerseMenu(' + ident + '); "><img src="' + theme + 'menu.png" class="menu" /></a></h4><p>' + jsonObject["data"][key].text + '</p></div>').trigger('create');
							counter++;
						}
 ...
 ********************************************/
function addSelected() {
	$("#" + localStorage.currentSection + "Button").addClass("selected-" + localStorage.currentTheme);
}
 ...
 ********************************************/
function removeSelected() {
	$("#" + localStorage.currentSection + "Button").removeClass("selected-" + localStorage.currentTheme);
}
/****************************************
 * function removeVerse()
 * Removes a verse a pack
...
function localize(theme) {
	if (!localStorage.localization) {
		...
		showLocal();
		applyTheme(localStorage.currentTheme || "a");
	}
...
function loadTheme() {
	...
	style.type = 'text/css';
	style.innerHTML = localStorage.theme;
	document.getElementsByTagName('head')[0].appendChild(style);
}

function applyTheme(themeClass) {
	$("#themepage").removeClass("ui-page-theme-a ui-page-theme-b").addClass("ui-page-theme-" + themeClass);
	$("#header").addClass("ui-bar-" + themeClass);

	if (themeClass == "a" || themeClass == "c" || themeClass == "d" || themeClass == "e" || themeClass == "g") {
		$("#knownImg").attr("src", "assets/img/dark/known.png");
		$("#workingImg").attr("src", "assets/img/dark/working.png");
		$("#allImg").attr("src", "assets/img/dark/all.png");
		$("#packImg").attr("src", "assets/img/dark/pack.png");
		$("#settingsImg").attr("src", "assets/img/dark/settings.png");
	} else if (themeClass == "b" || themeClass == "f") {
		$("#knownImg").attr("src", "assets/img/light/known.png");
		$("#workingImg").attr("src", "assets/img/light/working.png");
		$("#allImg").attr("src", "assets/img/light/all.png");
		$("#packImg").attr("src", "assets/img/light/pack.png");
		$("#settingsImg").attr("src", "assets/img/light/settings.png");
	}
}